...
#sql("isSuperAdmin")
	select accountId from gd_account_role
...
### 验证是否拥有某个 role
#sql("hasRole")
	select ar.accountId from gd_account_role ar
...
#sql("hasPermission")
	select ar.accountId from (
		select rp.roleId from gd_role_permission rp
		inner join gd_permission p on rp.permissionId = p.id
		where p.actionKey = #para(0)
	)
	as t inner join gd_account_role ar on t.roleId = ar.roleId
