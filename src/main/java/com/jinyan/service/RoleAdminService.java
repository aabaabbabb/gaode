...
public class RoleAdminService {
	...
	public Page<Role> paginate(int pageNum) {
		return dao.paginate(pageNum, pageSize, "select *", "from gd_role order by id asc");
	}
	...
	public List<Role> getAllRoles() {
		return dao.find("select * from gd_role order by id asc");
	}

	/**
	 * 判断角色名是否存�?
	 * @param roleId 当前 role 对象�? id 号，如果 role 对象还未创建，提供一个小�? 0 的�?�即�?
	 * @param name 角色�?
	 */
	public boolean exists(int roleId, String name) {
		name = name.toLowerCase().trim();
		String sql = "select id from gd_role where lower(name) = ? and id != ? limit 1";
	...
	public Ret save(Role role) {
		if (exists(-1, role.getName())) {
			return Ret.fail("角色名称已经存在，请输入别的名称");
		}
	...
	public Ret delete(final int roleId) {
		if (roleId == 1) {
			return Ret.fail("超级管理员不能被删除");
		}

		Db.tx(() -> {
			Db.delete("delete from gd_account_role where roleId=?", roleId);
			Db.delete("delete from gd_role_permission where roleId=?", roleId);
	...
	public Ret addPermission(int roleId, int permissionId) {
		if (roleId == 1) {
			return Ret.fail("超级管理员天然拥有所有权限，无需分配");
		}
	...
	public Ret deletePermission(int roleId, int permissionId) {
		if (roleId == 1) {
			return Ret.fail("超级管理员天然拥有所有权限，不能删除权限");
		}

		Db.delete("delete from gd_role_permission where roleId=? and permissionId=?", roleId, permissionId);
		return Ret.ok("msg", "删除权限成功");
	}

	/**
	 * 标记�? role 拥有的权限，用于在界面输�? checkbox �? checked 属�??
	 * 未来�? permission left join role_permission 来优�?
	 */
	public void markAssignedPermissions(Role role, List<Permission> permissionList) {
		...
		String sql = "select roleId from gd_role_permission where roleId=? and permissionId=? limit 1";
		for (Permission permission : permissionList) {
