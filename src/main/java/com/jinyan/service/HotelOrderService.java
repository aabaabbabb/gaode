...
public class HotelOrderService {
	...
	/*
	 * 艺龙状态参考
	 * array('F'=>'已入住','C'=>'已结帐','H'=>'变更','E'=>'取消','O'=>'满房','G'=>'变价','W'=>'虚拟'
	 * ,'Z'=>'删除,另换酒店','I'=>'大单','P'=>'暂无价格','R'=>'预付','U'=>'特殊满房','T'=>'计划中','B1'=>
	 * '有预定未查到','B2'=>'待查','B3'=>'暂不确定','J'=>'仅酒店已确认','V'=>'已审','S'=>'特殊','M'=>'恶意',
	 * 'N'=>'新单','A'=>'已确认','D'=>'删除','B'=>'NO SHOW')
	 */
...
//	private Order orderDao = new Order().dao();
	...
	private static String redis_perfix_order_biz_content = "redis_perfix_order_biz_content_"; // 存在 ledisdb 中的 键
	...
	public String createOrder(String biz_content) throws Exception {
		...
		if (order != null) {
			if (StrKit.isBlank(order.getEstatus())) {
				order.setEstatus("V");
			}
		}
...
//		SimpleDateFormat now= new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		this.getXiaoDianJiaGaoDe("https://api.xiaodianjia.cn/gdapi/create/order?gdid=" + id);
		String jedis_key = redis_perfix_order_biz_content + DateKit.toStr(new Date(), "yyyy-MM-dd HH:mm:ss")
				+ AmapOrderID;
		Jedis jedis = RedisUtil.getConn();
		jedis.set(jedis_key, biz_content);
		jedis.expire(jedis_key, 86400 * 3);

		jedis.lpush("gaodeList", id + "");

		jedis.close();

		/*
		 * 其它地方，如果需要完整 biz_content ，可以这样取 String b=jedis.get(jedis_key);
		 */
...
//		this.orderNotice(String.valueOf(id));
	...
	public  void getXiaoDianJiaGaoDe(String uri) throws Exception {
	...
	public boolean cancleOrder(String biz_content) throws Exception {
		...
		if (order != null) {
			if (!order.getEstatus().equals("E")) {
				...
				this.getXiaoDianJiaGaoDe("https://api.xiaodianjia.cn/gdapi/update/orderUnStatus?gdid=" + order.getId());
				//HttpKit.get("https://api.xiaodianjia.cn/gdapi/update/orderUnStatus?gdid=" + order.getId());
			}
			return true;
		}
		return false;
...
//					int cut_type = jsonObjectcancelShow.getIntValue("cut_type");
	...
	public List<Record> getOrderChangedList(String StartTime, String EndTime) {
		String sql = "select * from gd_order_change where   `update` >='" + StartTime + "' and   `update` <= '"
				+ EndTime + "' ";
...
class OrderXdjChangeLogThread extends Thread {
	...
	public void run() {
		...
		String url = "http://api.tetuijiudian.com/shuia/index.php?m=gaode&a=xdjstatuschg&id=" + id + "&estatus="
				+ estatus;
...
class AmapOrderPushThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			String sqlPara = "select price.*" + " from gd_ebooking_price as price " + " where price.hotel_id='"
					+ hotel.getElongId() + "' " + " and price.price_plan_id=" + order2.getRatePlanID() + " "
...
//			qstr=qstr+"&pricesDetails="+dayPrice+"&cancelShow="+cancelShowTxt  ;
			...
			try {
				// 这个是更新下订单的用户id
				String ret2 = HttpKit.post("https://api.tetuijiudian.com/shuia/index.php?m=gaode&a=addOrderSync", null,
						qstr, headers);
...
//		String ret = HttpKit.post("http://call.quchuchai.cn/Gaode/addOrder", null, dataMap.toString(),headers);
				...
				HttpKit.get(
						"http://api.tetuijiudian.com/shuia/index.php?g=content&m=exedlt&a=logincheck&txt=" + sendtxt);
			}
